...
//!
//! Embeds the gateway JS at compile time, extracts it to `~/.librefang/whatsapp-gateway/`,
//! runs `npm install` if needed, and spawns `node index.js` as a managed child process
//! that auto-restarts on crash.

use crate::config::librefang_home;
...
fn gateway_dir() -> PathBuf {
    librefang_home().join("whatsapp-gateway")
}
...
pub async fn start_whatsapp_gateway(kernel: &Arc<super::kernel::LibreFangKernel>) {
    ...
    let wa_config = match cfg.channels.whatsapp.first() {
        Some(c) => c.clone(),
        None => return,
    };
    ...
    let api_listen = &cfg.api_listen;
    let librefang_url = format!("http://{api_listen}");
...
mod tests {
    ...
    #[test]
    fn test_gateway_dir_under_librefang_home() {
        let dir = gateway_dir();
    ...
    fn test_write_if_changed_creates_new_file() {
        let tmp = std::env::temp_dir().join("librefang_test_gateway");
