...
mod tests {
    ...
    fn test_channels_hot_reload() {
        let a = default_cfg();
        let mut b = default_cfg();
        // Change the channels config by adding a Slack config (Discord
        // was migrated to a sidecar; Slack is the in-process fixture).
        b.channels.slack =
            librefang_types::config::OneOrMany(vec![librefang_types::config::SlackConfig {
