...
//!
//! This module detects both formats and converts them to LibreFang `SkillManifest`.
...
/// Result of converting a SKILL.md into LibreFang format.
#[derive(Debug, Clone)]
pub struct ConvertedSkillMd {
    ...
    /// Tool name translations applied (openclaw_name → librefang_name).
...
/// Full conversion of a SKILL.md directory to LibreFang format.
///
...
pub fn convert_skillmd(dir: &Path) -> Result<ConvertedSkillMd, SkillError> {
    ...
    if let Some(ref meta) = frontmatter.metadata.openclaw {
        ...
        // Convert commands to LibreFang tool definitions
        for cmd in &meta.commands {
            ...
            let librefang_name = if let Some(mapped) = tool_compat::map_tool_name(&cmd.name) {
                tool_translations.push((cmd.name.clone(), mapped.to_string()));
                mapped.to_string()
            } else if tool_compat::is_known_librefang_tool(&cmd.name) {
                cmd.name.clone()
            } else {
...
/// Convert an in-memory SKILL.md string into LibreFang format.
///
...
/// Write an LibreFang skill.toml manifest for an OpenClaw skill.
