//! Verifies the Gemini driver surfaces caller identity (agent / session /
//! step) on outbound HTTP requests as `x-librefang-*` headers, so any
//! observability sidecar in front of the Gemini API can correlate request
...
/// through to `GeminiDriver::with_emit_caller_trace_headers(false)`,
...
async fn complete_suppresses_trace_headers_when_emit_flag_disabled() {
    ...
    use uuid::Uuid;
    let driver = GeminiDriver::with_proxy_and_timeout(
