...
impl ChannelAdapter for NostrAdapter {
    ...
    > {
        ...
        info!(
            ...
            librefang_types::truncate_str(&pubkey, 16)
        );
                    ...
                    let should_reconnect = loop {
                        let msg = tokio::select! {
                            _ = relay_shutdown_rx.changed() => {
                                ...
                                // Send CLOSE
                                let close_msg = serde_json::json!(["CLOSE", "librefang-sub"]);
                                let _ = write.send(
                        ...
                        let mut channel_msg = ChannelMessage {
                            ...
                            sender: ChannelUser {
                                ...
                                display_name: format!(
                                    "{}...",
                                    librefang_types::truncate_str(&sender_pubkey, 8)
                                ),
                                librefang_user: None,
                            },
                            content: msg_content,
...
mod tests {
    ...
    fn test_nostr_build_subscription() {
        ...
        assert!(sub.contains("librefang-sub"));
