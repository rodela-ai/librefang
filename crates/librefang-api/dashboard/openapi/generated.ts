 ...
 *
 * STATUS (#5140): NOT imported anywhere in `src/`. The hand-maintained
 * types in `src/api.ts` are the canonical, de-facto source consumed by
 * the SPA. This file is a regenerable cross-reference only — refresh it
 * with `pnpm openapi:types` (requires a running daemon on :4545) when
 * you want a typed diff against the live OpenAPI schema; it is
 * intentionally retained, not wired into the build.
 */
