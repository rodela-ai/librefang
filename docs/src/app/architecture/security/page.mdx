...
18. [Dashboard Authentication](#18-dashboard-authentication)
19. [TOTP Second-Factor Verification](#19-totp-second-factor-verification)
20. [Security Configuration](#20-security-configuration)
21. [Security Dependencies](#21-security-dependencies)
...
|---|--------|-------|------------------|
| 1 | Capability-Based Security | `librefang-types` | Unauthorized actions by agents |
| 2 | WASM Dual Metering | `librefang-runtime` | Infinite loops, CPU DoS |
| 3 | Merkle Audit Trail | `librefang-runtime` | Tampered audit logs |
| 4 | Taint Tracking | `librefang-types` | Prompt injection, data exfiltration |
| 5 | Ed25519 Manifest Signing | `librefang-types` | Supply chain attacks |
| 6 | SSRF Protection | `librefang-runtime` | Server-Side Request Forgery |
| 7 | Secret Zeroization | `librefang-runtime`, `librefang-channels` | Memory forensics, key leakage |
| 8 | OFP Mutual Auth | `librefang-wire` | Unauthorized peer connections |
| 9 | Security Headers | `librefang-api` | XSS, clickjacking, MIME sniffing |
| 10 | GCRA Rate Limiter | `librefang-api` | API abuse, denial of service |
| 11 | Path Traversal Prevention | `librefang-runtime` | Directory traversal attacks |
| 12 | Subprocess Sandbox | `librefang-runtime` | Secret leakage via child processes |
| 13 | Prompt Injection Scanner | `librefang-skills` | Malicious skill prompts |
| 14 | Loop Guard | `librefang-runtime` | Stuck agent tool loops |
| 15 | Session Repair | `librefang-runtime` | Corrupted LLM conversation history |
| 16 | Health Endpoint Redaction | `librefang-api` | Information leakage |
| 17 | TOTP Second-Factor | `librefang-kernel` | Compromised channel self-approval |
...
### 7.6 SSRF Allowlist (self-hosted / K8s)

Self-hosted and Kubernetes deployments often need agents to reach internal
services (e.g., a private API behind a corporate proxy).  The
`[tools.web_fetch] ssrf_allowed_hosts` option lets operators explicitly
whitelist CIDRs or hostnames that are otherwise blocked by the private-IP
check.

**Cloud metadata ranges (`169.254.0.0/16`, `100.64.0.0/10`) are
unconditionally blocked and cannot be added to the allowlist.**
...
[tools.web_fetch]
ssrf_allowed_hosts = [
  "10.0.0.0/8",                # CIDR — entire RFC-1918 /8
  "172.16.0.0/12",             # CIDR — RFC-1918 /12
  "*.internal.example.com",    # glob prefix wildcard
  "svc.cluster.local",         # literal hostname
  "192.168.1.100",             # literal IP
]
```

#### Matching rules

| Entry format | Example | Matched against |
|---|---|---|
| CIDR | `"10.0.0.0/8"` | Resolved IP of the target host |
| Glob prefix | `"*.internal.example.com"` | Hostname in the URL (suffix match) |
| Literal IP | `"192.168.1.100"` | Resolved IP of the target host |
| Literal hostname | `"svc.cluster.local"` | Hostname in the URL (case-insensitive) |

#### What stays blocked

Even with a matching allowlist entry, the following are **always** rejected:

- `169.254.0.0/16` — link-local / AWS EC2 instance metadata
- `100.64.0.0/10` — CGNAT (includes Alibaba Cloud IMDS at `100.100.100.200`)
- All entries in the hostname blocklist (`localhost`, `metadata.google.internal`, etc.)
...
### 9.5 Wire Confidentiality

OFP frames are **plaintext** on the wire. The HMAC framing above proves
*who* the peer is and detects tampering or replay, but does **not** encrypt
the payload. Confidentiality is delegated to the deployment layer rather
than re-implemented inside `librefang-wire`. See
[OFP Wire Encryption](/architecture/ofp-wire) for the deployment recipe
(WireGuard / Tailscale / SSH tunnel / service-mesh mTLS), the threat model
that HMAC already covers, and the conditions under which we'd revisit.
...
### 18.2 Credential Sources (Priority Order)
...
### 18.3 Vault Storage (Recommended)
...
**Keyring fallback in containers and headless servers**: When the OS keyring is unavailable (Docker, CI, headless Linux), the daemon falls back to a file-based AES-256-GCM wrapped store. The wrapping key is derived via Argon2id from two independent entropy sources: a randomly-generated 32-byte machine-id persisted to `~/.local/share/librefang/.machine-id` (mode 0600) and available OS machine-identity material (`/etc/machine-id` on Linux, the IOPlatformUUID blob on macOS). Both sources are mixed with SHA-512 before the Argon2id KDF step.

**Container deployments should always set `LIBREFANG_VAULT_KEY`**: In environments where the `~/.local/share/librefang/.machine-id` file does not survive container restarts (e.g. ephemeral rootfs), the file-based fallback cannot provide strong security guarantees. Set `LIBREFANG_VAULT_KEY` to a 32-byte base64-encoded secret (generate with `openssl rand -base64 32`) and inject it via your secrets manager. The daemon emits a `WARN`-level log on startup whenever the OS keyring is unavailable, whether `LIBREFANG_VAULT_KEY` is set or not.
...
## 19. TOTP Second-Factor Verification

### 19.1 Threat Model

The approval gate sends confirmation requests to notification channels
(Telegram, Slack, etc.). If the originating channel is compromised, an attacker
can both initiate and approve dangerous operations — the approval gate provides
no protection.

TOTP (Time-based One-Time Password, RFC 6238) adds an **out-of-band** second
factor: the TOTP secret lives in an authenticator app on the operator's device,
completely independent of the messaging channel. Compromising the Telegram
account is not sufficient — the attacker also needs the authenticator app.
...
|-----------|----------|------|
| `SecondFactor` enum | `librefang-types/approval.rs` | Config: `none` or `totp` |
| `ApprovalManager::resolve()` | `librefang-kernel/approval.rs` | TOTP gate — rejects `Approved` without verification |
| `ApprovalManager::verify_totp_code()` | `librefang-kernel/approval.rs` | RFC 6238 verification (SHA-1, 6 digits, 30s step, ±1 window) |
| `ApprovalManager::generate_totp_secret()` | `librefang-kernel/approval.rs` | Secret generation for enrollment |
| Grace period cache | `ApprovalManager::totp_grace` | `HashMap<user_id, Instant>` — skip re-verification within window |
| Vault storage | `librefang-extensions/vault.rs` | TOTP secret stored as `totp_secret` key in AES-256-GCM vault |

### 19.3 Enrollment Flow

```
1. POST /api/approvals/totp/setup
   → Generates random TOTP secret (totp-rs Secret::generate_secret)
   → Stores in vault as "totp_secret" (not yet active)
   → Returns base32 secret + otpauth:// URI

2. User adds secret to authenticator app (Google Authenticator, 1Password, etc.)

3. POST /api/approvals/totp/confirm  { "code": "123456" }
   → Verifies code against stored secret
   → Sets "totp_confirmed" = "true" in vault

4. User sets second_factor = "totp" in config (hot-reloadable)
   → ApprovalManager.requires_totp() returns true
   → All future approvals require TOTP verification
```

### 19.4 Approval Flow with TOTP

```
1. Agent calls shell_exec → approval required
2. Kernel sends interactive notification to channel
   → Message includes: "TOTP required. Reply: /approve <id> <6-digit-code>"
   → Approve button hidden (code must be typed)
3. User replies: /approve abc123 654321
4. Channel bridge calls ApprovalManager::verify_totp_code(secret, "654321")
5. Code valid → resolve(Approved, totp_verified=true, user_id="channel_user")
6. Grace period recorded for user_id
7. Agent unblocked → tool executes
```

### 19.5 Grace Period

To avoid requiring a TOTP code on every approval, a configurable grace period
(`totp_grace_period_secs`, default 300s) allows skipping re-verification.

- Grace is tracked per `user_id` (not per source like "api" or "channel")
- `totp_grace_period_secs = 0` disables grace — every approval requires a code
- Maximum: 3600 seconds (1 hour)
- Grace cache is in-memory only — restarting the daemon resets it

### 19.6 Security Properties

| Property | Guarantee |
|----------|-----------|
| Secret storage | AES-256-GCM vault with Argon2id key derivation |
| Algorithm | TOTP (RFC 6238), SHA-1, 6 digits, 30s step |
| Clock skew tolerance | ±1 window (±30 seconds) |
| Verification layer | API/channel layer verifies before calling `resolve()` |
| Kernel enforcement | `resolve()` rejects `Approved` without `totp_verified=true` when TOTP enabled |
| Reject bypass | Reject/Deny decisions do NOT require TOTP (denying is always safe) |
| Batch limitation | Batch approve is blocked when TOTP is enforced |

---

## 20. Security Configuration

### 20.1 config.toml Reference
