...
For the still-in-process channels (Matrix, Mattermost, …), set
...
The default is `true`. The Discord, Slack, and Telegram sidecars
always chunk at their respective platform limits — they own their
own chunking inside the subprocess and the daemon-side knob does
not reach them.
