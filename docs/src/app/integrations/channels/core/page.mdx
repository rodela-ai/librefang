...
Signal defaults to `OutputFormat::PlainText` because `signal-cli` renders Markdown asterisks and underscores literally. To re-enable Markdown output, override the format on the agent or channel — see [Signal Plain-Text Default](/integrations/channels#signal-plain-text-default) on the channels overview.

### Media Attachments

The Signal adapter delivers `Image`, `Voice`, `Video`, `Audio`, `Animation`, `File`, `FileData`, and `MediaGroup` content blocks. Media URLs are downloaded and base64-encoded into `base64_attachments` on `/v2/send`. Unsupported types (`Poll`, `Sticker`, …) degrade gracefully — text-only fallback or skip — rather than fail the whole reply. See [Signal Media Attachments](/integrations/channels#signal-media-attachments) for the full type list.
...
For the still-in-process channels (Slack, Mattermost, …), set
...
The default is `true`. The Discord and Telegram sidecars always chunk
at the 2 000 UTF-16-unit boundary — they own their own chunking
inside the subprocess and the daemon-side knob does not reach them.
