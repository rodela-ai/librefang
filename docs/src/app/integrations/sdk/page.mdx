...
All four SDKs (`sdk/python/`, `sdk/javascript/`, `sdk/go/`, `sdk/rust/`) are mechanically generated from `openapi.json` by `scripts/codegen-sdks.py`. A pre-commit hook (`scripts/hooks/pre-commit`) re-runs the generator whenever the OpenAPI spec changes, so the SDKs never drift from the live API.
