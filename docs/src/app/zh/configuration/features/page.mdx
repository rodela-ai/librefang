...
headers = [{name = "Authorization", value_env = "MCP_API_KEY"}]
```
...
| `http` | `url`（string）、`headers`（header 配置列表，可选） | Streamable HTTP 传输方式（MCP 规范 2025-03-26+）。客户端通过单个 HTTP 端点使用 POST 发送 JSON-RPC 请求，服务器可选择以 SSE 流或普通 JSON 响应。相比 `sse` 传输方式，`http` 不需要专用的 SSE 通道，更适合无状态部署和 serverless 环境。 |
