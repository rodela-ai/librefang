...
对仍为进程内适配器的渠道（Matrix、Mattermost 等），可在渠道配置中设置 `split_long_messages = false` 以截断而非拆分长消息：
...
默认值为 `true`。Discord、Slack 与 Telegram sidecar 始终在各自平台限制处拆分——拆分逻辑由 sidecar 子进程内部处理，守护进程侧的开关对其无效。
