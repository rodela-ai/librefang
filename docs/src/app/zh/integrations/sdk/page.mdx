...
四个 SDK（`sdk/python/`、`sdk/javascript/`、`sdk/go/`、`sdk/rust/`）都由 `scripts/codegen-sdks.py` 从 `openapi.json` 机械生成。pre-commit hook（`scripts/hooks/pre-commit`）在 OpenAPI spec 变化时重跑生成器，所以 SDK 永远不会与线上 API 漂移。
