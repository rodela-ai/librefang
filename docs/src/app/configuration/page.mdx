...
| `workspaces_dir` | path or null | `null` | Root directory for agent workspaces. Defaults to `~/.librefang/workspaces`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `corp_id` | string | `""` | WeCom corporation ID. |
| `agent_id` | string | `""` | WeCom application agent ID. |
| `secret_env` | string | `"WECOM_SECRET"` | Env var holding the WeCom application secret. |
| `webhook_port` | u16 | `8454` | Port for the incoming webhook. |
| `token_env` | string or null | `null` | Env var holding the callback verification token. Required for verifying incoming messages. |
| `encoding_aes_key_env` | string or null | `null` | Env var holding the encoding AES key for message encryption/decryption. |
| `default_agent` | string or null | `null` | Agent name to route messages to. |
| `account_id` | string or null | `null` | Unique bot instance identifier for multi-bot routing. |
| `overrides` | object | (defaults) | Per-channel behavior overrides. See [Channel Overrides](#channel-overrides). |
...
| `hooks.ingest` | string or null | `null` | Python script path for the `ingest` hook (called on new user message). |
| `hooks.after_turn` | string or null | `null` | Python script path for the `after_turn` hook (called after each completed turn). |
