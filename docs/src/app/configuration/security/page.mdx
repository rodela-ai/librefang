...
| `LINE_CHANNEL_SECRET` | LINE (sidecar) | LINE channel secret (`librefang.sidecar.adapters.line` — set in `~/.librefang/secrets.env`). |
| `LINE_CHANNEL_ACCESS_TOKEN` | LINE (sidecar) | LINE channel access token (sidecar adapter). |
