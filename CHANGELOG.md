...
- **CHANGELOG attribution release gate** (`changelog-attribution-full` CI job): `scripts/check-changelog-attribution.py --all-unreleased` now runs on every PR and push to `main`, scanning the entire `[Unreleased]` block — not just what the diff adds. Catches entries that slipped past the pre-commit `--staged` check (via `--no-verify`, web editor, or CI auto-bump). The script gains `# pragma: no-attribution` support: bullets carrying that inline comment are explicitly exempted (use sparingly, for entries added before the convention was enforced). (#3307) (@houko)
...
- **Migrate remaining hand-rolled `printf`-style tables in `librefang-cli` to `crate::table::Table`** (#3306 2/N): `cmd_workflow_list`, `cmd_trigger_list`, `cmd_skill_list`, skill-version history, `cmd_channel_list`, `cmd_hand_list`, `cmd_hand_active`, `cmd_models_list` (both daemon and standalone branches), `cmd_models_aliases` (array + object + standalone), `cmd_models_providers` (daemon + standalone), `cmd_approvals_list`, `cmd_cron_list`, audit-log table, memory KV list, paired-devices list, sessions list, webhooks list — 20 call sites migrated. All lists now auto-degrade to ASCII when stdout is piped, and column widths size to content instead of being hardcoded. (@houko)
